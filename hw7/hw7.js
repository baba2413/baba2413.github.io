...
async function initShader() {
    ...
    pShader = new Shader(gl, vertexShaderSource, fragmentShaderSource);

    const gVert = await readShaderFile('gVert.glsl');
    const gFrag = await readShaderFile('gFrag.glsl');
    gShader = new Shader(gl, gVert, gFrag);
...
async function main() {
    try {
        ...
        setCommonUniforms(pShader);
        setCommonUniforms(gShader);
...
function setCommonUniforms(sh) {
    sh.use();
    sh.setMat4("u_projection", projMatrix);

    sh.setVec3("material.diffuse",  [1.0, 0.5, 0.31]);
    sh.setVec3("material.specular", [0.5, 0.5, 0.5]);
    sh.setFloat("material.shininess", 16);

    sh.setVec3("light.position",  lightPos);
    sh.setVec3("light.ambient",   [0.2, 0.2, 0.2]);
    sh.setVec3("light.diffuse",   [0.7, 0.7, 0.7]);
    sh.setVec3("light.specular",  [1.0, 1.0, 1.0]);
    sh.setVec3("u_viewPos", cameraPos);
}
